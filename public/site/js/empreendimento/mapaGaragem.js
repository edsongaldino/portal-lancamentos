$(document).ready(function () {
  ...
  */
  ...
  // inicio - ponto garagem
  $(".ponto_garagem").click(function (e) {
    ponto = false;
    
    if($(this).attr("data-stgaragem") == "r") {
      $("#modal_detalhes").find(".modal-header").addClass("modal-header-situacao-r");

      $("#modal_detalhes").find("h4").html('<i class="fa fa-car" aria-hidden="true"></i> Reservada');      
    } else if($(this).attr("data-stgaragem") == "d") {
      $("#modal_detalhes").find(".modal-header").addClass("modal-header-situacao-d");

      $("#modal_detalhes").find("h4").html('<i class="fa fa-car" aria-hidden="true"></i> Disponível');
      
    } else if($(this).attr("data-stgaragem") == "v") {
      $("#modal_detalhes").find(".modal-header").addClass("modal-header-situacao-v");

      $("#modal_detalhes").find("h4").html("Vendido");      
    } else if($(this).attr("data-stgaragem") == "o") {
      $("#modal_detalhes").find(".modal-header").addClass("modal-header-situacao-o");

      $("#modal_detalhes").find("h4").html("Bloqueada");
    }else if($(this).attr("data-stgaragem") == "b") {
    ...
    ajaxRequest({
      ...
      metodo: 'POST',
      dados: {
        garagem: $(this).attr("data-idgaragem"),
        ocultar: 'N'
      },
      feedback: false,
      resultado: '#box_detalhes_garagem'
    });
  ...
  $("#tipo_ponto").on("change", function () {
    $("#vinculo_garagem").hide();
    $("#vinculo_imagem").hide();
    $("#vinculo_garagem").find("input, select").removeAttr("required");
    $("#vinculo_imagem").find("input, select").removeAttr("required");
    
    if($("#tipo_ponto").val() == "U") {
      $("#vinculo_garagem").show();
      $("#vinculo_garagem").find("input, select").attr("required", "required");
      $("#ponto_garagem").focus();
