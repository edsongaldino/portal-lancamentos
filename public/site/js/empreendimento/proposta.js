...
Proposta.prototype.onParcelasMensais = function() {  
  var $this = this;
  $(document).on('blur', '#quantidade_parcela,#valor_parcela', function () {    
    $this.setValorTotalMensais();      
    $this.atualizar();
  });
};
...
Proposta.prototype.setValorTotalMensais = function () {
  ...
  ? 0
  : this.remover_mascara($("#valor_parcela").val());

  var qtde = $("#quantidade_parcela").val() == "" 
  ? 0
  : this.remover_mascara($("#quantidade_parcela").val());
...
Proposta.prototype.onParcelaBalao = function() {
  var $this = this;
  $(document).on('blur', '#parcela_balao', function () {
    $this.setValorTotalBaloes();        
  });
};
...
Proposta.prototype.onTipoNegociacao = function () {
  $(document).on('change', '#tipo_negociacao_saldo2', function () {    
    ...
    <div class="row" id="remove' + x +'">\
      ...
      <div class="col-sm-4">\
        <div class="input-group mb-md">\
          ...
          <input type="text" name="valor_parcela_balao[]" class="form-control valor valor_parcela_balao_' + x +'" id="parcela_balao">\
        </div>\
      </div>\
      <div class="col-sm-2">\
      ...
      <div class="col-sm-2 botoes-balao">\
          <a href="javascript:void(0)" class="simple-ajax-modal btn btn-lg btn-danger btn-remove-parcela remove_campo" id="' + x +'">\
