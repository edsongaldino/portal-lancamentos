  ...
  $(".ponto_garagem").click(function (e) {
      ponto = false;
      $("#info_marcacao").html("Unidade " + $(this).attr("data-garagem"));
      $("#form2 #garagem_id").val($(this).attr("data-idgaragem"));
  ...
  $("#bt_modal_atualizar").on('click', function (e) {
    ...
    if ($("#tipo_ponto").val() == 'V') {
        var url = '/admin/empreendimento/garagem/atualizar-coordenadas';
    }
    
    if ($("#tipo_ponto").val() == 'V') {

        var garagem_id = $("select[name=garagem_id]").val();

        if (garagem_id == '') {
          Swal.fire({
            title: "Erro",
            text: 'Selecione uma garagem',
            type: 'error'
          });
          return;
        }
    ...
    promise.then(function () {
      ...
      setTimeout(function () {
        window.open("/admin/empreendimento/" + id + "/mapa-garagens/?x=" + px + "&y=" + py + "", "_self");
      }, 2000);      
    });    
  });
  ...
  $("#bt_modal_excluir").on('click', function (e) {
    ...
    var promise = new Promise(function (resolve, reject) {
      ajaxRequest({
        url: '/admin/empreendimento/garagem/atualizar-coordenadas',
        metodo: 'POST',
